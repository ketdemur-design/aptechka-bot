...
from telegram.ext importimport os
import asyncio
import re
import json
from datetime import datetime, timedelta
...
async def post_init(application):
    """Вызывается сразу после инициализации приложения (правильный способ для PTB ≥ 20)."""
    await application.bot.set_my_commands([
        BotCommand("start", "перезапустить бота"),
    ])
    load_data_store()
    # Создаём фоновую задачу через application.create_task — она управляется PTB,
    # и Task was destroyed не возникает при остановке.
    application.create_task(reminder_loop(application))
    print(f"Бот v{BOT_VERSION} инициализирован. DATA_FILE={DATA_FILE}")
...
def main():
    app = (
        ApplicationBuilder()
        .token(BOT_TOKEN)
        .post_init(post_init)   # ← правильный способ для PTB ≥ 20
        .build()
    )
